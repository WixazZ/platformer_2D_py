...
if __name__ == '__main__':
    ...
    for i in carte.structure:
        ...
        def update(self):
            self.speed = vec(0, 0.5)
            self.turbo.x = 0
            pressed = pg.key.get_pressed()
            if pressed[pg.K_s]: self.speed.y = 0.8
            if pressed[pg.K_a]: self.turbo.x = -9
            if pressed[pg.K_d]: self.turbo.x = 9
            if self.pos.x < 0:
                self.pos.x = 0
            if self.pos.x > 1600:
                self.pos.x = 1600
            if self.i != 50:
                self.i = self.i + 1
            if pressed[pg.K_SPACE] and pressed[pg.K_a] and self.i == 50:
                self.i = 0
                projectil = Projectil(self.pos.x - 40, self.pos.y - 40, -20, 'boule_de_glace.png')
                all_project.add(projectil)
                all_sprite.add(projectil)
            if pressed[pg.K_SPACE] and pressed[pg.K_d] and self.i == 50:
                self.i = 0
                projectil = Projectil(self.pos.x + 20, self.pos.y - 40, 20, 'boule_de_glace.png')
                all_project.add(projectil)
                all_sprite.add(projectil)
            if self.pos.y <= 10:
                self.j = self.j + 1
